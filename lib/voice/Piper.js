...
const FS = require("fs");
const Streamer = require('../util/Streamer');
        ...
        if(typeof source === "string") {
            if(options.format === "dca" || options.format === "ogg" || options.format === "webm" || options.format === "pcm") {
                if(source.startsWith("http://") || source.startsWith("https://")) {
                    ...
                    // TODO Parse URL, append Range header in options based on received data. Maybe passthrough.on(data) and Buffer.byteLength?

                    Streamer.openStream(source, passThrough);
                    //if(source.startsWith("http://")) {
                    //    HTTP.get(source, (res) => res.pipe(passThrough)).once("error", (e) => this.stop(e));
                    //} else {
                    //    HTTPS.get(source, (res) => res.pipe(passThrough)).once("error", (e) => console.log(e) /*this.stop(e)*/);
                    //}
                    source = passThrough;
                } else {
                    try {
